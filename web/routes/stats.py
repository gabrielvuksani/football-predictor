...
async def api_dashboard_stats():
    ...
    try:
        total_preds = db.execute(
            "SELECT COUNT(*) FROM predictions WHERE model_version = 'v12_analyst'"
        ).fetchone()[0] or 0

        avg_conf = db.execute(
            "SELECT AVG(GREATEST(p_home, p_draw, p_away)) FROM predictions WHERE model_version = 'v12_analyst' AND p_home IS NOT NULL"
        ).fetchone()[0] or 0.5

        top_pred = db.execute("""
            SELECT m.home_team, m.away_team, p.p_home, p.p_draw, p.p_away
            FROM predictions p JOIN matches m ON p.match_id = m.match_id
            WHERE p.model_version = 'v12_analyst' AND p.p_home IS NOT NULL
        ...
        accuracy_row = db.execute("""
            ...
            WHERE ps.model_version = 'v12_analyst' AND m.status = 'FINISHED'
...
async def api_stats():
    ...
    try:
        ...
        top_pred = db.execute("""
            ...
            WHERE p.model_version = 'v12_analyst'
        ...
        avg_conf_row = db.execute("""
            ...
            FROM predictions WHERE model_version = 'v12_analyst' AND confidence IS NOT NULL
        """).fetchone()
...
async def api_performance(model: str = Query("v12_analyst")):
