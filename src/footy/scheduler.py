...
- pre_match_refresh: Refresh zero-cost feeds shortly before kickoff windows
- train_base: Train Elo + Poisson models
- train_council: Train council model (v10) with 20 experts + meta-learner
...
class TrainingScheduler:
    ...
    def _get_job_func(self, job_type: str):
        ...
        job_funcs = {
            ...
            "pre_match_refresh": self._job_pre_match_refresh,
    ...
    def _job_pre_match_refresh(self, lookahead_days: int = 2) -> dict:
        """Refresh near-term fixtures and regenerate predictions before kickoff windows."""
        results: dict[str, int] = {}
        results["rows_ingested"] = pipeline.ingest(days_back=2, days_forward=lookahead_days, verbose=False)
        try:
            results["clubelo_updates"] = pipeline.refresh_external_elo(verbose=False)
        except Exception as e:
            log.debug("pre_match_refresh clubelo warning: %s", e)
            results["clubelo_updates"] = 0
        con = connect()
    ...
    def _job_score(self) -> dict:
        ...
        try:
            from footy.pipeline import score_finished_predictions
            result = score_finished_predictions(verbose=True)
            # After scoring, update ensemble weights (self-learning loop)
            try:
                from footy.continuous_training import ContinuousTrainingManager
                mgr = ContinuousTrainingManager()
                ensemble_result = mgr.learn_ensemble_weights(lookback_days=90)
                result["ensemble_weights_updated"] = ensemble_result.get("status") == "updated"
                # Also track per-expert accuracy for self-learning
                expert_result = mgr.track_expert_accuracy(verbose=True)
                result["experts_tracked"] = len(expert_result)

                # Check and trigger retraining if ready
                retrain_check = mgr.check_and_retrain("v15_architect")
                retrain_status = retrain_check.get("v15_architect", {})
                if retrain_status.get("status") in ("ready_to_retrain", "drift_detected"):
                    ...
                    if retrain_result.get("action") in ("deployed", "rolled_back"):
                        result["retrain_action"] = retrain_result.get("action")
                        result["retrain_improvement_pct"] = retrain_result.get("improvement_pct", 0)
            except Exception as ew_err:
                log.warning("Ensemble weight / expert learning / retraining failed: %s", ew_err)
            return {"scored_predictions": result.get("scored", 0)}
        except Exception as e:
            log.error("Scoring job failed: %s", e)
            return {"error": str(e), "scored_predictions": 0}

    def _job_retrain(self, force: bool = False) -> dict:
        """Auto-retrain using continuous_training.ContinuousTrainingManager.auto_retrain()."""
        try:
            from footy.continuous_training import ContinuousTrainingManager
            mgr = ContinuousTrainingManager()
