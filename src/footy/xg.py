...
def learn_conversion_rates(con: duckdb.DuckDBPyConnection) -> dict:
    ...
    for comp, goals, sot, shots, n_matches in rows:
        ...
        # Compute on_target_rate directly from actual data
        on_target_rate = goals / sot if sot > 0 else 0.10
        # Clamp on_target_rate to reasonable range [0.05, 0.20]
        on_target_rate = max(0.05, min(0.20, on_target_rate))
        # Compute off_target_rate using regression: (goals - sot * on_target_rate) / off_target
        remaining_goals = max(0, goals - sot * on_target_rate)
        off_target_rate = remaining_goals / off_target if off_target > 0 else 0.0
    ...
    if agg_sot > 0:
        ...
        overall_on = max(0.05, min(0.20, overall_on))
        overall_off_target = agg_shots - agg_sot
        overall_remaining_goals = max(0, agg_goals - agg_sot * overall_on)
        overall_off = overall_remaining_goals / overall_off_target if overall_off_target > 0 else 0.0
        overall_off = max(0.005, min(0.05, overall_off)) if overall_off_target > 0 else 0.02
    else:
