...
def ingest_extras_fdcuk(n_seasons: int = 8, verbose: bool = True) -> int:
    ...
    Respects tracked_competitions to avoid unnecessary ingestion.
    """
    ...
    # Only ingest extras for tracked competitions that have FDCUK mappings
    tracked = set(cfg.tracked_competitions)
    divs = [d for comp, d in DIV_MAP.items() if comp in tracked]
    if not divs:
        divs = list(DIV_MAP.values())  # fallback: all if none tracked
