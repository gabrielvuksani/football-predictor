...
def walk_forward_cv(
    ...
    *,
    ...
    Args:
        ...
        embargo_days: Gap between train end and test start to reduce leakage
    ...
    for fold_i, (test_start, test_end) in enumerate(fold_cutoffs):
        ...
        is_last_fold = fold_i == len(fold_cutoffs) - 1
        if is_last_fold:
            test_mask = (dates >= test_start) & (dates <= test_end)
        else:
            test_mask = (dates >= test_start) & (dates < test_end)

        train_cutoff = test_start - pd.Timedelta(days=max(0, embargo_days))
        if expanding:
            train_mask = dates < train_cutoff
        else:
            if max_train_months:
                train_start = train_cutoff - pd.Timedelta(days=max_train_months * 30)
