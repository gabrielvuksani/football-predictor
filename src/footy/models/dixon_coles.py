...
def fit_dc(
    ...
    Args:
        ...
        max_goals: Maximum goals to model for scorelines
        l2: L2 regularization coefficient

    Returns:
        Fitted DCModel or None if insufficient data or validation fails
    """
    # Input validation
    if df is None or df.empty or len(df) < 250:
        return None

    required_cols = {"utc_date", "home_team", "away_team", "home_goals", "away_goals"}
    ...
    if df["home_goals"].isna().any() or df["away_goals"].isna().any():
        raise ValueError("home_goals and away_goals must not contain NaN values")

    if (df["home_goals"] < 0).any() or (df["away_goals"] < 0).any():
        raise ValueError("Goals must be non-negative")

    if not pd.api.types.is_numeric_dtype(df["home_goals"]) or not pd.api.types.is_numeric_dtype(df["away_goals"]):
        raise ValueError("home_goals and away_goals must be numeric")
    ...
    def nll(x):
        ...
        tau[m10] = 1.0 + lam[m10] * rho   # τ(1,0): home scores 1 → lam
        tau[m01] = 1.0 + mu[m01] * rho     # τ(0,1): away scores 1 → mu
        tau[m11] = 1.0 - rho
