...
class SkellamRegressionExpert(Expert):
    ...
    def compute(self, df: pd.DataFrame) -> ExpertResult:
        ...
        for i, r in enumerate(df.itertuples(index=False)):
            ...
            # Regression: goal rates from covariates
            # λ₁ (home goals) increases with Elo advantage and attack strength
            log_lam_h = (
                0.30  # home advantage intercept
                + 0.40 * elo_diff
                + 0.15 * form_diff
                + 0.30 * (att_h / self.AVG_GOALS - 1.0)
                + 0.20 * (def_a / self.AVG_GOALS - 1.0)  # weak opponent defense
            )
            log_lam_a = (
                0.15  # away intercept (lower)
                - 0.40 * elo_diff
                - 0.15 * form_diff
                + 0.30 * (att_a / self.AVG_GOALS - 1.0)
                + 0.20 * (def_h / self.AVG_GOALS - 1.0)
            )
