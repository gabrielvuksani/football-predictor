...
class PoissonExpert(Expert):
    ...
    def compute(self, df: pd.DataFrame) -> ExpertResult:
        ...
        for i, r in enumerate(df.itertuples(index=False)):
            ...
            # --- update ---
            # F3 fix: Only update rolling state for finished matches
            ...
            if hg is not None and ag is not None:
                ...
                if len(rh["hg"]) >= _RHO_MIN_SAMPLES and len(rh["hg"]) % 200 == 0:
                    try:
                        ...
                        old_rho = _rho_cache[comp]
                        new_rho = estimate_rho_mle(
                            np.array(rh["hg"]), np.array(rh["ag"]),
                            np.array(rh["lh"]), np.array(rh["la"]),
                        )
                        _rho_cache[comp] = new_rho
                        log.debug(f"Updated rho for {comp}: {old_rho:.4f} -> {new_rho:.4f}")
                    except Exception as e:
                        log.warning(f"Failed to update rho for {comp}: {e}")  # keep previous estimate
