...
def fit_poisson(matches: pd.DataFrame, halflife_days: float = 180.0) -> dict:
    ...
    Args:
        matches: DataFrame with columns home_team, away_team, home_goals, away_goals, utc_date
        halflife_days: Time decay half-life in days

    Returns:
        Dictionary with teams, attack, defense, home_adv, and mu parameters
    """
    # Input validation
    if matches is None or len(matches) == 0:
        return {"teams": [], "attack": [], "defense": [], "home_adv": 0.0, "mu": 0.0}

    required_cols = {"home_team", "away_team", "home_goals", "away_goals", "utc_date"}
    if not required_cols.issubset(matches.columns):
        raise ValueError(f"DataFrame missing required columns: {required_cols - set(matches.columns)}")
    ...
    if not res.success and res.fun >= 1e10:
        raise ValueError(f"Optimization failed to converge with objective value {res.fun}")

    mu, ha, att, deff = unpack(res.x)

    # Validate final parameters
    if not np.all(np.isfinite(att)) or not np.all(np.isfinite(deff)):
        raise ValueError("Optimization produced non-finite parameters (NaN or Inf)")
    ...
    if not (-5.0 <= float(mu) <= 5.0):
        raise ValueError(f"Invalid mu value {mu}; expected in reasonable range [-5, 5]")
    if not (-1.5 <= float(ha) <= 1.5):
        raise ValueError(f"Invalid home_adv value {ha}; expected in range [-1.5, 1.5]")
