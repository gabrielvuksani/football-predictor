...
def list_jobs():
    ...
    for job in jobs:
        ...
        _status = (job["last_status"] or "").upper()
        status_color = "green" if _status == "SUCCESS" else ("red" if _status == "FAILED" else "yellow")
...
def history(job_id: str, limit: int = 10):
    ...
    for run in hist:
        ...
        _run_status = (run["status"] or "").upper()
        status_color = "green" if _run_status == "SUCCESS" else ("red" if _run_status == "FAILED" else "yellow")
