...
def export(
    ...
    rows = db.execute(f"""
        ...
        LEFT JOIN predictions p
          ON m.match_id = p.match_id AND p.model_version = 'v12_analyst'
    ...
    _dump(api / "matches.json", {"matches": matches, "model": "v12_analyst"})
    ...
    # ── 7. Insights / review / training payloads used by static tabs ──
    insights = _build_static_insights(db, comp_list)
    _dump(api / "btts-ou.json", insights["btts_ou"])
    _dump(api / "accumulators.json", {"accumulators": insights["accumulators"]})
    _dump(api / "accuracy.json", insights["accuracy"])
    _dump(api / "post-match-review.json", insights["post_match_review"])
    _dump(api / "training" / "status.json", insights["training_status"])
    for comp, table in insights["form_tables"].items():
        _dump(api / "form-table" / f"{comp}.json", {
            "competition": comp,
            "last_n": 6,
            "table": table,
        })
    for comp, preview in insights["round_previews"].items():
        _dump(api / "round-preview" / f"{comp}.json", preview)
    console.print("  insights/ review/ training static payloads")
...
def _export_match_detail(db, api: Path, match_id: int) -> None:
    ...
    pred_row = db.execute(
        ...
        "FROM predictions WHERE match_id=? AND model_version='v12_analyst'",
        [match_id],
    ).fetchone()
    prediction = None
    poisson_stats = None
    ...
    score_row = db.execute(
        ...
        "WHERE match_id=? AND model_version='v12_analyst'",
        [match_id],
    ).fetchone()
    score = None
    if score_row:
...
def _build_value_bets(
    ...
    """Identify value bets from the matches list with Kelly sizing and risk metrics.

    Integrates Kelly criterion for optimal bet sizing and includes confidence-adjusted
    edge, expected value calculations, and model agreement-based risk ratings.

    Args:
        matches: List of match dicts with predictions and odds
        min_edge: Minimum probability edge to consider (0-1)
        use_kelly: Whether to include Kelly criterion sizing
        kelly_frac: Fraction of full Kelly to use (e.g., 0.25 = 25% Kelly)

    Returns:
        List of value bet dicts, sorted by edge descending
    """
    from footy.models.kelly import (
        kelly_fraction as compute_kelly_frac,
        expected_value,
    ...
    for m in matches:
        ...
        model_agreement = m.get("model_agreement", 0.5)
        ...
        for outcome, prob_key, odds_key, label in [
            ...
            edge = compute_edge(prob, impl_prob)
            ...
            ev_pct = expected_value_percentage(prob, odds)

            # Confidence-adjusted edge and EV
            confidence_factor = 1.0 + (confidence - 0.5) * 0.5
            conf_adjusted_edge = edge * confidence_factor
            conf_adjusted_ev = ev * confidence_factor

            # Kelly sizing
            kelly_pct = 0.0
            kelly_stake = 0.0
            if use_kelly:
                kelly_pct = compute_kelly_frac(
                    prob, odds, kelly_fraction=1.0, max_fraction=0.5
                )
                kelly_stake = kelly_pct * kelly_frac

            # Risk rating based on model agreement
            # Low agreement = high risk (models disagree on this prediction)
            ...
            bets.append({
                ...
                "odds": round(odds, 2),
                "edge": round(edge, 4),
                "confidence_adjusted_edge": round(conf_adjusted_edge, 4),
...
def _build_training_status(db) -> dict:
    """Build training status payload matching live `/api/training/status`."""
    from footy.continuous_training import ContinuousTrainingManager

    drift = None
    expert_rankings = []
    mgr = None
    active_version = "v12_analyst"
    try:
        mgr = ContinuousTrainingManager()
        drift = mgr.detect_drift("v12_analyst")
    ...
    try:
        rows = db.execute("""
            SELECT model_type, retrain_threshold_matches, performance_threshold_improvement,
        ...
        for r in rows:
            schedules.append({
                "model_type": r[0],
                "threshold_matches": r[1],
                "threshold_improvement": r[2],
                "last_trained": str(r[3])[:19] if r[3] else None,
                "pending_matches": mgr.get_new_matches_since_training(r[0]) if mgr else r[4],
            })
    except Exception:
        pass
    ...
    try:
        rows = db.execute("""
            SELECT model_version, model_type, training_window_days,
                   ...
                   deployed, training_date, improvement_pct
        ...
        for r in rows:
            ...
            history.append({
                ...
                "improvement_pct": round(float(r[8]) * 100, 2) if r[8] is not None else None,
            })
    except Exception:
        pass
...
def _build_static_insights(db, comp_list: list[str]) -> dict:
    """Build static payloads for tabs that the GitHub Pages UI already renders."""
    from footy.llm.insights import (
        btts_ou_insights,
        build_accumulators,
        league_form_table,
    ...
    payload = {
        "btts_ou": {"btts_likely": [], "btts_unlikely": [], "over25": [], "under25": []},
        "accumulators": [],
        "accuracy": {"total": 0},
        "post_match_review": {
            "matches_reviewed": 0,
            "correct": 0,
            "accuracy": 0,
            "review": "No recent finished matches with predictions.",
            "misses": [],
        },
        "training_status": _build_training_status(db),
        "form_tables": {},
        "round_previews": {},
    }

    try:
        payload["btts_ou"] = btts_ou_insights()
    except Exception as e:
        log.warning("Failed to build static BTTS/OU payload: %s", e)

    try:
        payload["accumulators"] = build_accumulators()
    except Exception as e:
        log.warning("Failed to build static accumulators payload: %s", e)

    try:
        payload["accuracy"] = prediction_accuracy_stats(days_back=30)
    except Exception as e:
        log.warning("Failed to build static accuracy payload: %s", e)

    try:
        payload["post_match_review"] = post_match_review(days_back=7)
    except Exception as e:
        log.warning("Failed to build static post-match review payload: %s", e)

    for comp in comp_list:
        try:
            payload["form_tables"][comp] = league_form_table(comp, last_n=6)
        except Exception as e:
            log.warning("Failed to build static form table for %s: %s", comp, e)
            payload["form_tables"][comp] = []
        try:
            payload["round_previews"][comp] = league_round_summary(comp)
        except Exception as e:
            log.warning("Failed to build static round preview for %s: %s", comp, e)
            payload["round_previews"][comp] = {
                "competition": comp,
                "matches": 0,
                "summary": "No upcoming matches with predictions.",
                "predictions": [],
            }
