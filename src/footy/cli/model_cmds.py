...
def backtest(days: int = 365, test_days: int = 28):
    ...
    from footy.models.council import prepare_training_data
    con = connect()
    X, y, dates = prepare_training_data(con, days=days)
    if X is None or len(X) == 0:
        console.print("[red]No training data available for backtest[/red]")
        return
    results = walk_forward_cv(X, y, dates, n_folds=4, verbose=True)
...
def record(
    ...
    console.print(f"  Improvement: {result['improvement_pct'] * 100:+.2f}%")
...
def history(model_type: str, limit: int = 10):
    ...
    for run in hist:
        ...
        table.add_row(
            ...
            f"[{improvement_color}]{run['improvement_pct'] * 100:+.2f}%[/{improvement_color}]",
