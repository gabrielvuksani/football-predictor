...
# Skip entire module if DB doesn't exist or has no match data
_DB = os.path.join(os.path.dirname(__file__), "..", "data", "footy.duckdb")

def _db_has_matches():
    """Check if the live database exists AND contains match data."""
    if not os.path.exists(_DB):
        return False
    try:
        import duckdb
        c = duckdb.connect(_DB, read_only=True)
