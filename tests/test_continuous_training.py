...
def _seed_expert_cache(con):
    """Insert finished-match expert breakdowns for expert self-learning tests."""
    ...
    rows = [
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.62, "draw": 0.22, "away": 0.16}, "confidence": 0.71},
                    "zip_model": {"probs": {"home": 0.58, "draw": 0.24, "away": 0.18}, "confidence": 0.66},
                }
            },
        ),
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.41, "draw": 0.36, "away": 0.23}, "confidence": 0.62},
                    "zip_model": {"probs": {"home": 0.30, "draw": 0.47, "away": 0.23}, "confidence": 0.69},
                }
            },
        ),
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.52, "draw": 0.24, "away": 0.24}, "confidence": 0.57},
                    "zip_model": {"probs": {"home": 0.20, "draw": 0.25, "away": 0.55}, "confidence": 0.74},
                }
            },
        ),
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.56, "draw": 0.23, "away": 0.21}, "confidence": 0.64},
                    "zip_model": {"probs": {"home": 0.54, "draw": 0.24, "away": 0.22}, "confidence": 0.68},
                }
            },
        ),
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.45, "draw": 0.28, "away": 0.27}, "confidence": 0.55},
                    "zip_model": {"probs": {"home": 0.22, "draw": 0.24, "away": 0.54}, "confidence": 0.72},
                }
            },
        ),
        (
            ...
            {
                "experts": {
                    "elo": {"probs": {"home": 0.38, "draw": 0.34, "away": 0.28}, "confidence": 0.59},
                    "zip_model": {"probs": {"home": 0.25, "draw": 0.50, "away": 0.25}, "confidence": 0.76},
                }
            },
        ),
    ]
    for idx, (match_id, hg, ag, breakdown) in enumerate(rows):
        con.execute(
            "INSERT INTO matches VALUES (?, 'FINISHED', ?, ?, ?, ?)",
            [match_id, hg, ag, base + timedelta(hours=idx), "PL"],
        )
        con.execute(
            "INSERT INTO expert_cache VALUES (?, ?)",
            [match_id, json.dumps(breakdown)],
        )
...
class TestRecordTraining:
    def test_record_first_training(self, manager):
        ...
        assert result["improvement_pct"] == 1.0  # First deploy — no baseline
    ...
    def test_record_improvement(self, manager):
        ...
        assert result["improvement_pct"] == pytest.approx(0.05, abs=0.001)
...
# ---------------------------------------------------------------------------
# Expert self-learning
# ---------------------------------------------------------------------------

class TestExpertSelfLearning:
    def test_track_expert_accuracy(self, manager):
        _seed_expert_cache(manager.con)
        result = manager.track_expert_accuracy()
        assert "elo" in result
        assert "zip_model" in result
        assert result["zip_model"]["n"] == 6
        assert result["zip_model"]["accuracy"] >= result["elo"]["accuracy"]

    def test_get_expert_rankings(self, manager):
        _seed_expert_cache(manager.con)
        manager.track_expert_accuracy()
        rankings = manager.get_expert_rankings()
        assert len(rankings) >= 2
        assert rankings[0]["expert"] == "zip_model"
        comp_rankings = manager.get_expert_rankings(competition="PL")
        assert comp_rankings[0]["expert"] == "zip_model"

    def test_get_expert_weight_map(self, manager):
        _seed_expert_cache(manager.con)
        manager.track_expert_accuracy()
        weights = manager.get_expert_weight_map()
        assert weights
        assert abs(sum(weights.values()) - 1.0) < 1e-9
        assert weights["zip_model"] > weights["elo"]
