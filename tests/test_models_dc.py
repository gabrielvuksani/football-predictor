...
class TestDixonColes:
    ...
    def test_tau_low_scores(self):
        ...
        # 1-0: tau = 1 + lam*rho → uses HOME lambda, not away
        t10 = _tau(1, 0, 2.0, 0.5, -0.1)
        assert t10 == pytest.approx(1.0 + 2.0 * (-0.1))  # 0.8

        # 0-1: tau = 1 + mu*rho → uses AWAY lambda, not home
        t01 = _tau(0, 1, 2.0, 0.5, -0.1)
        assert t01 == pytest.approx(1.0 + 0.5 * (-0.1))  # 0.95

        # Verify asymmetry: with different lambdas, 1-0 and 0-1 must differ
        assert t10 != t01
