...
class TestGoalPatternExpert:
    ...
    def test_expert_count(self):
        from footy.models.council import ALL_EXPERTS
        assert len(ALL_EXPERTS) >= 20  # 20+ experts (22 core + TrueSkill if available)

    def test_new_experts_exist(self):
        from footy.models.council import ALL_EXPERTS
        names = [type(e).__name__ for e in ALL_EXPERTS]
        assert "NetworkStrengthExpert" in names
        assert "ZIPExpert" in names
